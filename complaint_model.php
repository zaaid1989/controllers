...
class Complaint_model extends CI_Model
  {
	...
	public function is_allowed($role)
	  ...
	  public function current_status($status)
	  {
		  ...
		  if( $status=="Pending SPRF"){ 
			  echo '<span class="label label-sm bg-purple"> Completed </span>';
		  }
		  if( $status=="SPRF Approved"){ 
			  echo '<span class="label label-sm label-danger"> Completed </span>';
		  }
		  if( $status=="N/A"){ 
	...
	{
			$dbres = $this->db->query("SELECT * FROM tbl_clients   order by `pk_client_id` DESC");
            $dbresResult=$dbres->result_array();
            return $dbresResult;
	}
